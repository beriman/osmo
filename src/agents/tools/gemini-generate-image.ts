...
export function createGeminiGenerateImageTool(options?: {
  ...
  return {
    ...
    description: "Generate an image using Google Gemini (Imagen). Provide a detailed prompt.",
    ...
    execute: async (_toolCallId, args) => {
      ...
      // Google Generative AI Imagen API
      // Note: This is a placeholder for the actual Imagen API call.
      // Depending on the exact model (Gemini Pro vs Imagen 3), the endpoint might differ.
      // For Gemini Pro, image generation is often via a specific model name.
      
      const modelId = "imagen-3.0-generate-001"; // Or "gemini-1.5-pro-002" if it supports it
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:predict?key=${auth.apiKey}`;
